...
depends_on:
  - index.md
  - program-identity.md
  - program-state-machine.md
  - progress-ttl-scheduling.md
decision_refs:
...
| DeepSeek Harness | Provider-private headers are not a supported identity source. | DSH supplies task, session, role, and lineage through canonical `vllm_xargs.agentic_context` (or `agent_hint` where that contract applies). |
...
See [Core tuning parameters](progress-ttl-scheduling.md#core-tuning-parameters) for formulas and measurement guidance.
