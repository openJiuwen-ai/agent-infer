...
from agentcache.benchmarks.benchkit.config import RequestProxyConfig
from agentcache.benchmarks.request_proxy.lifecycle import (
    RequestProxyHandle,
    RequestProxyLifecycle,
    _available_port,
)
from agentcache.benchmarks.request_proxy.process import ProcessExit, RequestProxyProcess
from agentcache.benchmarks.request_proxy.server import RequestProxyLaunchConfig, run_request_proxy
...
def test_process_uses_spawn_context_target_and_config(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("agentcache.benchmarks.request_proxy.process.multiprocessing.get_context", get_context)
...
def test_lifecycle_start_readiness_authenticated_shutdown_join_and_idempotent_close(
    ...
    async def run():
        ...
        monkeypatch.setattr("agentcache.benchmarks.request_proxy.lifecycle.httpx.AsyncClient", lambda **_kwargs: client)
...
def test_lifecycle_cancellation_terminates_child(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    async def run():
        ...
        monkeypatch.setattr(
            "agentcache.benchmarks.request_proxy.lifecycle.httpx.AsyncClient",
...
def test_lifecycle_reports_early_exit_and_uses_failure_termination(
    ...
    async def run():
        ...
        monkeypatch.setattr(
            "agentcache.benchmarks.request_proxy.lifecycle.httpx.AsyncClient", lambda **_kwargs: FakeClient([])
        )
